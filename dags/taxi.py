...
with DAG(
    ...
    params = {
        "run_month": Param(
            ...
            type        = "string",
            pattern     = r"^(\d{4}-\d{2})?$",
            description = "Nếu rỗng, pipeline dùng logical_date; nếu đặt, nạp đúng tháng này",
        )
    },
) as dag:
        ...
        configuration = {
            "load": {
                ...
                "destinationTable": {
                    ...
                    "tableId":   "yellow_trips${{ (params.run_month or ds)[:7]|replace('-', '') }}"
                },
        ...
        configuration = {
            "load": {
                ...
                "destinationTable": {
                    "projectId": "{{ params.project_id }}",
                    "datasetId": "nyc_taxi_raw",
                    "tableId":   "green_trips${{ (params.run_month or ds)[:7]|replace('-', '') }}"
                },
                "writeDisposition": "WRITE_TRUNCATE",
                "sourceFormat":     "PARQUET",
                "autodetect":       False
    ...
    dbt_deps >> dbt_seed
    dbt_seed  >> [load_yellow_to_bq, load_green_to_bq]
